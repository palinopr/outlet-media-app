...
const VISIBILITY_OPTIONS: CampaignActionItemVisibility[] = ["shared", "admin_only"];

interface CampaignActionItemFormProps {
  campaignId: string;
  clientSlug: string;
  defaultStatus?: TaskStatus;
  item?: CampaignActionItem | null;
...
export function CampaignActionItemForm({
  campaignId,
  clientSlug,
  defaultStatus = "todo",
  item,
  ...
  const [visibility, setVisibility] = useState<CampaignActionItemVisibility>(
    (item?.visibility ?? "shared") as CampaignActionItemVisibility,
  );
    ...
    startTransition(async () => {
      try {
        ...
        if (isEditing && item) {
          ...
          await createCampaignActionItem({
            assigneeId: null,
            assigneeName: toNullableString(assigneeName),
            campaignId,
            clientSlug,
        ...
        setError(
          submitError instanceof Error ? submitError.message : "Failed to save action item.",
        );
      }
    });
  };
      ...
      <DialogContent className="border-[#ece8df] bg-[#fcfbf8] text-[#2f2f2f] sm:max-w-lg">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <Label htmlFor="campaign-action-title">Title</Label>
            <Input
          ...
          <div className="space-y-2">
            <Label htmlFor="campaign-action-description">Description</Label>
            <Textarea
              id="campaign-action-description"
              value={description}
              onChange={(event) => setDescription(event.target.value)}
              placeholder="Optional context, handoff details, or blockers."
              rows={4}
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={visibility}
                onValueChange={(value) =>
                  setVisibility(value as CampaignActionItemVisibility)
                }
              >
                <SelectTrigger className="border-[#e5ded2] bg-white">
                  <SelectValue />
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="campaign-action-assignee">Assignee</Label>
              <Input
                id="campaign-action-assignee"
                value={assigneeName}
            ...
            <div className="space-y-2">
              <Label htmlFor="campaign-action-due-date">Due date</Label>
              <Input
                id="campaign-action-due-date"
                type="date"
                value={dueDate}
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            >
              {isPending ? "Saving..." : isEditing ? "Save changes" : "Create action"}
            </Button>
          </div>
        </form>
      </DialogContent>
