...
const VISIBILITY_OPTIONS: AssetFollowUpItemVisibility[] = ["shared", "admin_only"];

interface AssetFollowUpItemFormProps {
  assetId: string;
  clientSlug: string;
  defaultStatus?: TaskStatus;
  item?: AssetFollowUpItem | null;
...
export function AssetFollowUpItemForm({
  assetId,
  clientSlug,
  defaultStatus = "todo",
  item,
  ...
  const [visibility, setVisibility] = useState<AssetFollowUpItemVisibility>(
    (item?.visibility ?? "shared") as AssetFollowUpItemVisibility,
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-[#ece8df] bg-[#fcfbf8] text-[#2f2f2f] sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{isEditing ? "Edit asset follow-up" : "New asset follow-up"}</DialogTitle>
        </DialogHeader>

        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <Label htmlFor="asset-follow-up-title">Title</Label>
            <Input
              id="asset-follow-up-title"
              value={title}
              onChange={(event) => setTitle(event.target.value)}
              placeholder="What should happen next with this creative?"
              required
          ...
          <div className="space-y-2">
            <Label htmlFor="asset-follow-up-description">Description</Label>
            <Textarea
              id="asset-follow-up-description"
              value={description}
              onChange={(event) => setDescription(event.target.value)}
              placeholder="Optional review notes, production details, or blockers."
              rows={4}
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={visibility}
                onValueChange={(value) => setVisibility(value as AssetFollowUpItemVisibility)}
              >
                <SelectTrigger className="border-[#e5ded2] bg-white">
                  <SelectValue />
                </SelectTrigger>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="asset-follow-up-assignee">Assignee</Label>
              <Input
                id="asset-follow-up-assignee"
                value={assigneeName}
            ...
            <div className="space-y-2">
              <Label htmlFor="asset-follow-up-due-date">Due date</Label>
              <Input
                id="asset-follow-up-due-date"
                type="date"
                value={dueDate}
