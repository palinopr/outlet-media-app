...
import { CampaignDiscussionForm } from "./campaign-discussion-form";

interface CampaignOperatingPanelProps {
  campaignId: string;
  data: ClientCampaignOperatingView;
  slug: string;
}
...
function actionItemPriorityTone(priority: keyof typeof TASK_PRIORITY_LABELS) {
...
function groupDiscussionThreads(comments: CampaignComment[]) {
  const repliesByParent = new Map<string, CampaignComment[]>();
...
export function CampaignOperatingPanel({
  campaignId,
  data,
  slug,
