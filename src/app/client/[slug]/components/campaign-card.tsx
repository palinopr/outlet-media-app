  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-1">
        <p className="text-sm font-semibold text-white leading-tight group-hover:text-cyan-300 transition-colors">{c.name}</p>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 mt-auto">
        <div>
          <p className="text-xs text-white/50 mb-0.5">Spend</p>
          <p className="text-lg font-bold text-white">{fmtUsd(c.spend)}</p>
        </div>
        <div>
          <p className="text-xs text-white/50 mb-0.5">ROAS</p>
          <p className={`text-lg font-bold ${roasColor(c.roas)}`}>
            {c.roas != null ? `${c.roas.toFixed(1)}x` : "--"}
          </p>
        </div>
        <div>
          <p className="text-xs text-white/50 mb-0.5">Revenue</p>
          <p className="text-sm font-bold text-emerald-400">{fmtUsd(c.revenue)}</p>
        </div>
        <div>
          <p className="text-xs text-white/50 mb-0.5">Impressions</p>
          <p className="text-sm font-bold text-white/90">{fmtNum(c.impressions)}</p>
        </div>
        <div>
          <p className="text-xs text-white/50 mb-0.5">Clicks</p>
          <p className="text-sm font-bold text-white/90">{fmtNum(c.clicks)}</p>
        </div>
        {c.ctr != null && (
          <div>
            <p className="text-xs text-white/50 mb-0.5">CTR</p>
            <p className="text-sm font-bold text-white/90">{c.ctr.toFixed(2)}%</p>
          </div>
        )}
      </div>
      ...
      {c.roas != null && (
        <div className="mt-4 pt-3 border-t border-white/[0.06]">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className={`text-xs font-semibold ${roasColor(c.roas)}`}>{roasLabel(c.roas)}</span>
          </div>
