...
- Keep client `Agent` on the local-worker architecture: the web app should queue read-only client-agent turns into durable app state, and the Claude-powered local worker on the operator machine should pick them up through the worker gateway, execute against normalized client-safe analytics tools, and write the reply back into the same thread.
