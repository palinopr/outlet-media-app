# Table: crm_comments

Generated from the current working tree on 2026-04-10 21:27:09.

- Category: Other tables
- Kinds: table
- Migrations: supabase/migrations/20260306100000_crm_comments.sql, supabase/migrations/20260306213000_client_shared_workflow_rls.sql
- Non-migration references: 3
- Referenced by groups: src/app / admin (2), Root Files (1)
- Routes: none
- Features: none
- Shared libs: none
- Agent files: none
- Mutation-oriented files: src/app/admin/actions/clients.ts
- Tests: src/app/admin/clients/data.test.ts
- Docs: none
- Other mentions: audit/architecture-smells.md, src/app/admin/actions/clients.ts
- Behavior signals from references: calls revalidatePath() (1), server action/module (1)
- Auth signals from references: references membership/scope access concepts (3), imports Clerk server auth (1)

## Reference files
- audit/architecture-smells.md, src/app/admin/actions/clients.ts, src/app/admin/clients/data.test.ts
