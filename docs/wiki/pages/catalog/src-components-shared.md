# src/components / shared

Generated from the current working tree on 2026-04-28 03:23:46.

- Files: 1
- File kinds: React/TSX module (1)

Each entry below documents the file path, system ownership, construction style, imports/exports when available, cross-links to tests and routes, and a concise contents summary.

## `src/components/shared/error-boundary.tsx`
- Status: tracked-clean
- System: web
- Group: src/components / shared
- Ownership: shared app components
- Type: React/TSX module
- Construction: component/UI-oriented module, contains `use client`
- Lines: 38
- Bytes: 1092
- Imports (internal): src/components/ui/button.tsx, src/lib/client-error-reporting.ts
- Imports (packages): react
- Imported by: src/components/admin/error-boundary.tsx, src/components/client/error-boundary.tsx
- Depends on groups: src/components / ui, src/lib
- Used by groups: src/components / admin, src/components / client
- Exports: ErrorBoundary
- Symbol details: function ErrorBoundary (exported)
- Defines: ErrorBoundary
- Contents summary: contains `use client`; exports: ErrorBoundary; internal imports: 2; package imports: 1
