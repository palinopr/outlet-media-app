...
Skills provide specialized instructions for specific tasks. **You must load the relevant skill before acting.** This is not optional.

🚨 **MANDATORY — EVERY SESSION, BEFORE ANY RESPONSE:**
Load `using-superpowers` skill FIRST. Before exploring, before answering, before asking questions. No exceptions.

| When... | Load skill... |
|---------|---------------|
| **Always — first action of every session** | `using-superpowers` |
| Starting work in a new/unfamiliar project | `learn-codebase` |
| Making git commits (every commit — no exceptions) | `commit` |
...
| Running dev servers, background tasks, or processes in separate terminals | `cmux` |
| Adding or configuring an MCP server | `add-mcp-server` |
| Facing 2+ independent tasks that can run in parallel | `dispatching-parallel-agents` |
| Executing implementation plans with subagents | `subagent-driven-development` |
| About to claim work is complete/fixed/passing | `verification-before-completion` |
...
| Creating a multi-step implementation plan | `writing-plans` |
| Starting feature work that needs isolation | `using-git-worktrees` |
| Completing a development branch (merge/PR decision) | `finishing-a-development-branch` |
| Any creative work — new features, components, functionality | `brainstorming` |
| Receiving code review feedback | `receiving-code-review` |
| Completing tasks and ready for review | `requesting-code-review` |
