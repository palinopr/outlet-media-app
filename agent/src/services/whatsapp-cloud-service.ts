...
async function ensureDiscordThread(
  ...
  await sendAsAgent(CUSTOMER_AGENT_KEY, channelName, {
    content: intro,
    threadId: thread.id,
  }).catch((e) => console.warn("[whatsapp] thread intro send failed:", toErrorMessage(e)));
...
async function mirrorInboundMessage(
  ...
  await sendAsAgent(CUSTOMER_AGENT_KEY, channelName, threadId ? { content, threadId } : content).catch((e) => console.warn("[whatsapp] mirror inbound failed:", toErrorMessage(e)));
...
async function notifyApprovalBlock(
  ...
  if (!context.approvalRequestedAt) {
    ...
    await sendAsAgent("boss", WHATSAPP_BOSS_CHANNEL, bossText).catch((e) => console.warn("[whatsapp] approval boss notify failed:", toErrorMessage(e)));
  }
  ...
  await sendAsAgent(
    ...
    threadId
      ...
      : `**Internal triage**\n${statusText}\n${instructions}`,
  ).catch((e) => console.warn("[whatsapp] approval block notify failed:", toErrorMessage(e)));
...
async function routeOwnerControlMessage(
  ...
  await sendAsAgent("boss", WHATSAPP_BOSS_CHANNEL, bossText).catch((e) => console.warn("[whatsapp] owner control notify failed:", toErrorMessage(e)));
...
async function maybeHandleSimpleDirectMessage(
  ...
  await sendAsAgent(
    ...
    threadId
      ...
      : `**Internal triage**\nSent direct WhatsApp reply.\n\n_${reply}_`,
  ).catch((e) => console.warn("[whatsapp] direct reply triage notify failed:", toErrorMessage(e)));
  ...
  return await withResourceLocks(task.id, [`whatsapp-conversation:${conversationId}`], async () => {
      ...
      if (gate.reason === "group-mention-only") {
        await sendAsAgent(
          ...
          threadId
            ...
            : "**Internal triage**\nIgnoring group chatter until the agent is explicitly mentioned.",
        ).catch((e) => console.warn("[whatsapp] group chatter notify failed:", toErrorMessage(e)));
        await markMessageTriaged(latestInbound.id);
    ...
    if (deliveredText) {
      await sendAsAgent(CUSTOMER_AGENT_KEY, channelName, threadId
        ...
        : `**Internal triage**\n${deliveredText}`,
      ).catch((e) => console.warn("[whatsapp] triage result send failed:", toErrorMessage(e)));
    }
